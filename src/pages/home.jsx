...
function Home() {
  return (
    <section className="relative bg-[#0B0B0B] text-white min-h-[calc(100vh-73px)] lg:h-[calc(100vh-73px)] flex flex-col justify-between overflow-hidden">

      {/* ── Subtle dark coffee tint wash (no bright gradient, no orbs) ── */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(60,42,28,0.10) 0%, transparent 60%)",
        }}
      />
      ...
      {/* Bottom vignette for grounding, kept neutral black */}
      ...
      <div className="relative flex-1 max-w-7xl mx-auto px-5 sm:px-8 md:px-12 lg:px-14 py-10 sm:py-12 md:py-14 lg:py-0 w-full grid md:grid-cols-2 items-center gap-10 md:gap-8 lg:gap-16">
        ...
        <div className="flex flex-col gap-5 sm:gap-6 z-10">
          ...
          {/* Eyebrow badge */}
          <div className="hero-fade-up delay-100 inline-flex flex-wrap items-center gap-2.5 w-fit max-w-full bg-white/[0.04] border border-white/10 text-[#c89b3c]/90 px-4 py-2 rounded-full text-[11px] sm:text-xs font-semibold tracking-[0.18em] uppercase">
          ...
          <div className="hero-fade-up delay-400 flex flex-wrap gap-3">
            <Link
              to="/menu"
              className="group relative text-center overflow-hidden bg-[#c89b3c] hover:bg-[#b88a2f] text-white font-bold px-7 py-3.5 sm:py-3 rounded-2xl text-sm tracking-wide transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(200,155,60,0.35)] active:translate-y-0"
            >
            ...
            <Link
              to="/about"
              className="text-center border border-white/15 hover:border-[#c89b3c]/40 text-white/65 hover:text-white px-7 py-3.5 sm:py-3 rounded-2xl text-sm hover:bg-white/[0.05] tracking-wide transition-all duration-300 hover:-translate-y-1"
            >
              About Us
            </Link>
          </div>
      ...
      {/* ── BOTTOM MARQUEE ── */}
      <div className="relative border-t border-white/[0.07] bg-white/[0.02] py-3 overflow-hidden flex-shrink-0 z-10">
